...
class Experiment(nn.Module):
    ...
    def run_test_phase(self, data=None, save_name="",model_name=None,load_model=True):
        ...
        model_name = model_name if model_name else "best_validation_model.pth.tar"
    ...
    def run_train_phase(self,save_name=""):
        ...
        self.save_model("best_validation_model{}.pth.tar".format(save_name))
        self.save_statistics(train_stats, 'training_summary{}.csv'.format(save_name))
        self.save_results(last_train_cm, 'training_results{}.csv'.format(save_name))
        self.save_statistics(val_stats, 'validation_summary{}.csv'.format(save_name))
        self.save_results(best_val_cm, 'validation_results{}.csv'.format(save_name))
